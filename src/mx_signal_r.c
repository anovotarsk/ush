...
static unsigned int out_monitor_and_read_keyboard(t_info *info, int index,
    ...
    if (index != MX_MAX_COMAND + 1)
        mx_monit_free_new(MX_NAME, mx_strlen(MX_COMMAND[index]) + 1, 0,
                           MX_COMMAND[index]);
    else
        mx_monit_free_new(MX_NAME, 1, 0, "");
    mx_printstr("\n");
    mx_monit_free_new(MX_SEARCH, i, 0, temp);
    ch = mx_getchar();
    mx_monit_clear_new(MX_SEARCH, i, 0, temp);
    mx_print_escape("1A");
    return ch;
}
...
static int search_comand(t_info *info, int index, char *temp) {
    ...
    if (index != MX_MAX_COMAND + 1) {
        ...
        mx_monit_clear_new(MX_NAME, str_len_comand, 0, MX_COMMAND[index]);
    }
    else
        mx_monit_clear_new(MX_NAME, 1, 0, "");
