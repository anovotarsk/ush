...
static char **many_comand_in_list(char *path, char **str, t_info *info,
    ...
    mx_monit_clear("", info, *str);
    mx_print_comand_with_tab(list_comand);
...
static void one_comand_in_list(t_info *info, t_list *list_comand, char **str,
    ...
    for (; len_comand != 0 && path[len_comand] != '/';)
        ...
        for (int i = len_comand - 1; path[i]; i++)
            mx_ush_symb_one(str, path[i], &(MX_STR_LEN), MX_STR_POS);
        if (MX_STR_POS != 0) {
...
char **mx_tab_param(char *parsing, char **str, t_info *info) {
    char *path = mx_pars_tab(parsing, info);
    t_list *list_comand = 0;
    char **creat_list_comand = NULL;
    int comand_len = 0;

    mx_ush_reading_comand(path, &list_comand);
