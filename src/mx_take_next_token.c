...
t_token* mx_take_next_token(int *currPos, int end, char *str,
                           t_info *processes) {
    ...
    if (*currPos >= end)
        return 0;
    if (mx_check_char(str[tokenStart]))
        newToken = mx_make_token_in_prog(currPos, end, str, processes);
    else if (is_operator(str[tokenStart])) {
        newToken = mx_make_token(2,0,0);
        ...
        newToken->type = mx_take_type_token(newToken->value[0]);
